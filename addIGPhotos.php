...
add_action('wp_ajax_nopriv_igPhoto', 'addIgPhotos');
add_action('wp_ajax_igPhoto', 'addIgPhotos');

function addIgPhotos()
{
    /*
    ...
    $instagramNextIndex = get_post_meta($orderId, 'next_instagram_index', true);
 ...
 */
    if (isset($_POST)) {
        ...
        foreach ($_POST as $p => $photo) {
            $instagramIndex = $i + $instagramNextIndex;
            $filename = "instagram" . $instagramIndex . ".jpg";
            ...
            $newInstagramIndex =  $i + $instagramNextIndex;
        }
        update_post_meta($orderId, "next_photo_index", $newNextPhotoIndex);
        update_post_meta($orderId, "next_instagram_index", $newInstagramIndex);
